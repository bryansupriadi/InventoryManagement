...
function ProductBrandsDetail() {
  const { group, item, subCategory: urlSubCategory, brandName: urlBrandName } = useParams();
  ...
  const filteredProducts = product.filter(
    (item) =>
      item['Sub Category'] === urlSubCategory && item['Brand Name'] === urlBrandName
  );
  ...
  return (
    <div className='App'>
      ...
      <div className='fab-btn-1'>
        <FloatingActionDetail group={group} item= {item} subCategory={urlSubCategory} brandName={urlBrandName}/>
      </div>
    </div>
  );
}
